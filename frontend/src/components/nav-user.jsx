  ...
  return (
    <div ref={popoverRef} className="relative w-full">
      ...
      <button
        ...
        className={`flex w-full items-center justify-between gap-2 p-1.5 rounded-xl border transition-all cursor-pointer text-left outline-none select-none group-data-[collapsible=icon]:p-1 group-data-[collapsible=icon]:size-9 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:mx-auto ${
          isOpen
      ...
      >
        ...
        <ChevronsUpDown className={`w-4 h-4 text-muted-foreground shrink-0 group-data-[collapsible=icon]:hidden mr-1 transition-transform duration-200 ${isOpen ? 'rotate-180 text-foreground' : ''}`} />
      </button>

      {/* Popover Menu Content */}
      {isOpen && (
        <div
          className={`absolute z-50 p-1.5 rounded-xl bg-popover text-popover-foreground border border-border shadow-2xl animate-in fade-in-0 zoom-in-95 duration-150 ${
            state === 'collapsed'
        ...
        >
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-xs font-bold text-destructive hover:bg-destructive hover:text-white transition-colors cursor-pointer text-left"
          >
