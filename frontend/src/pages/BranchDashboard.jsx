...
export function BranchDashboard() {
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/70 pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Pemantauan alokasi armada pool, supir, arus ritase logistik, dan konsumsi BBM per cabang/site operasional.
          </p>
        </div>
        ...
        <CardContent className="p-3">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none text-xs">
            ...
            <Button
              ...
              className={cn("h-8 rounded-lg text-xs font-bold shrink-0", selectedRegionId === 'all' && "bg-amber-500 text-slate-950 hover:bg-amber-400")}
            >
              Semua Wilayah (Overview)
            </Button>

            {overviewData?.regions?.map((reg) => (
              <Button
                key={reg.id}
                type="button"
      ...
      {/* ─── CONDITIONAL CONTENT: ALL REGIONS VS SINGLE REGION ───────────── */}
      {selectedRegionId === 'all' ? (
        /* ══════════════════════════════════════════════════════════════════
           MODE 1: SEMUA WILAYAH (OVERVIEW COMPARATIVE DASHBOARD)
           ══════════════════════════════════════════════════════════════════ */
        <div className="space-y-6">
                ...
                <CardHeader className="p-4 pb-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="space-y-0.5">
                      ...
                      <CardTitle className="text-sm font-bold group-hover:text-amber-500 transition-colors line-clamp-1">
                        {reg.name}
                      </CardTitle>
                    </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="border-border/80 shadow-xs">
              ...
              <CardContent className="p-5 pt-4 h-72">
                <Bar data={fuelCostChartData} options={comparisonChartOptions} />
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="border-border/80 shadow-xs">
            <CardHeader className="p-5 pb-3">
              <CardTitle className="text-sm font-bold flex items-center gap-2">
                ...
                Tabel Komparasi 8 Wilayah Operasional Tambang & Kantor
              </CardTitle>
              <CardDescription className="text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="border-border/80 shadow-xs">
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-foreground">{detailStats.vehicles_available}</span>
                  <span className="text-xs text-muted-foreground">Siap / {detailStats.vehicles_total} Unit</span>
                </div>
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground pt-1 border-t border-border/50">
                  <span>{detailStats.passenger_count} Penumpang</span>
                  <span>&middot;</span>
                  <span>{detailStats.cargo_count} Barang</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/80 shadow-xs">
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-foreground">{detailStats.drivers_available}</span>
                  <span className="text-xs text-muted-foreground">Siap / {detailStats.drivers_total} Orang</span>
                </div>
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground pt-1 border-t border-border/50">
                  <span className="text-blue-400 font-semibold">{detailStats.drivers_on_duty} Sedang Bertugas</span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-border/80 shadow-xs">
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-foreground">{detailStats.active_outgoing_count}</span>
                  <span className="text-xs text-muted-foreground">Keluar / {detailStats.active_incoming_count} Masuk</span>
                </div>
            ...
            <Card className="border-border/80 shadow-xs">
              <CardContent className="p-4 space-y-2">
                ...
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground pt-1 border-t border-border/50">
                  <span>{detailStats.monthly_fuel_liters?.toLocaleString('id-ID')} Liter Terpakai</span>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="border-border/80 shadow-xs">
              <CardHeader className="p-5 pb-2">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Truck className="w-4 h-4 text-amber-500" />
                  Komposisi Armada Pool ({detailData?.region?.name})
                </CardTitle>
                <CardDescription className="text-xs">
                  Distribusi kendaraan berdasarkan jenis armada dan kepemilikan.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-5 pt-2 h-64">
                <Doughnut data={detailFleetChartData} options={{ maintainAspectRatio: false, cutout: '65%', plugins: { legend: { position: 'bottom', labels: { color: '#a1a1aa', font: { size: 10 }, usePointStyle: true, pointStyle: 'circle' } } } }} />
              </CardContent>
            </Card>
            ...
            <Card className="border-border/80 shadow-xs">
              ...
              <CardContent className="p-5 pt-2 h-64">
                <Bar data={destinationRoutesChartData} options={comparisonChartOptions} />
              </CardContent>
            </Card>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 border-b border-border/70 pb-2">
              <Button
                ...
                className={cn("h-8 text-xs font-bold gap-1.5", activeDetailTab === 'fleet' && "bg-amber-500 text-slate-950 hover:bg-amber-400")}
              >
              ...
              <Button
                ...
                className={cn("h-8 text-xs font-bold gap-1.5", activeDetailTab === 'drivers' && "bg-amber-500 text-slate-950 hover:bg-amber-400")}
              >
              ...
              <Button
                ...
                className={cn("h-8 text-xs font-bold gap-1.5", activeDetailTab === 'trips' && "bg-amber-500 text-slate-950 hover:bg-amber-400")}
              >
              ...
              <Button
                ...
                className={cn("h-8 text-xs font-bold gap-1.5", activeDetailTab === 'fuel_service' && "bg-amber-500 text-slate-950 hover:bg-amber-400")}
              >
